...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/30 backdrop-blur-md border-b border-border/20 shadow-sm w-full">
      <div className="w-full px-4 sm:px-6 lg:px-8">
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <div className="md:hidden pb-4 space-y-2 bg-background/90 backdrop-blur-md rounded-lg mx-2 mb-2 border border-border/20">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
